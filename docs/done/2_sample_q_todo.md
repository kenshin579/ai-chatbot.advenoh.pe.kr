...
- [x] MCP Playwright로 다음 항목 확인:
  - [x] 초기 화면에 예시 질문 카드 4개 표시되는지
  - [ ] 예시 질문 카드 클릭 시 질문이 전송되는지 (백엔드 미실행으로 전송 테스트 생략)
  - [ ] 대화 시작 후 예시 질문 카드가 사라지는지 (백엔드 미실행으로 생략)
  - [x] 입력창이 둥근 모서리 + 내부 Send 아이콘으로 표시되는지
  - [x] 블로그 전환(blog-v2 → investment → inspireme) 시 예시 질문이 변경되는지
  - [x] 다크모드에서 카드/입력창 스타일 정상 표시되는지
